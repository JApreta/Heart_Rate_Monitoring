...
bool sendData(int r, int o){
    Rate3 = r;
    oxxxy = o;
    fullJson = String::format("{%d,%d,,%d,%d,d,%d,%d,%d}", r, o, Time.hour(),Time.minute(),Time.second(),Time.month(), Time.day(),Time.year());
    //publish wil return 0 if send is not successful, return back to main in order to catch errors:0
    if(!Particle.publish("toJudith", String::format("{%d,%d,%d,%d,%d,%d,%d,%d}", r, o, Time.hour(),Time.minute(),Time.second(),Time.month(), Time.day(),Time.year()))){
      olDatar[cNum] = r;
      cNum++;
      olDatao[cNum] = o;
      return 0;}
